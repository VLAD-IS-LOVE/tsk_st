...
function defProps(obj, key, value) {
    if (key in obj) {
...
function totalPrice(value) {
  ...
  function getDate(docDate) {
    return new Date(docDate.split(" ")[0]).toLocaleString("ru", {
      month: 'long',
      day: 'numeric'
    });
  }
  
  var formattedData = data.map(function(elem) {
    elem.docDate = getDate(elem.docDate);
    return elem;
  });
  
  var newData = formattedData.reduce(function(acc, value) {
    if (!(value.docDate in acc)) {
      var lst;
      acc[value.docDate] = (lst = {}, defProps(lst, value.id, {
        items: [{
          src: value.image,
          price: value.price,
          name: value.name,
      ...
      }), defProps(lst, "count", 1), defProps(lst, "documentPrice", totalPrice(value)), lst);
      Object.defineProperty(acc[value.docDate], "count", {
        enumerable: false
      });
      Object.defineProperty(acc[value.docDate], "documentPrice", {
        enumerable: false
      });
      Object.defineProperty(acc[value.docDate][value.id], "count", {
        enumerable: false
      });
      Object.defineProperty(acc[value.docDate][value.id], "type", {
        enumerable: false
      });
      Object.defineProperty(acc[value.docDate][value.id], "groupPrice", {
        enumerable: false
      });
    } else {
      if (!(value.id in acc[value.docDate])) {
        acc[value.docDate][value.id] = {
          items: [{
            src: value.image,
            price: value.price,
            name: value.name,
        ...
        var newItems = acc[value.docDate][value.id]["items"];
        newItems.push({
          src: value.image,
          price: value.price,
          name: value.name,
        ...
        acc[value.docDate][value.id] = {
          items: newItems,
          type: value.docType,
          count: acc[value.docDate][value.id]["count"] + 1,
          groupPrice: acc[value.docDate][value.id]["groupPrice"] + totalPrice(value)
        };
        Object.defineProperty(acc[value.docDate][value.id], "count", {
          enumerable: false
        });
  ...
  var res = nunjucks.renderString(template, {
    modify: newData
  }, function(err, res) {
